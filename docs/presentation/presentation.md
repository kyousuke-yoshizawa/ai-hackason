...
🔒 **実装**:
- `api/_lib/authz.ts` を新設し、Express（`server/`）と Vercel Functions（`api/`）の認可ロジックを一本化
...
- 調査の過程で `store_managers.manager_id` 等9カラムが誤って Supabase Auth の `auth.users` を参照しており、実運用でFK制約違反になる系統的バグを発見。関連分を修正しつつ、範囲外の7カラムは追加タスク（T23）として記録
