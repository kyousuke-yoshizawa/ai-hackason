...
**用途**:
1. **提案生成** — ユーザーの過去参加履歴から最適なアウティングプランを生成
2. **概要・説明文の自動作成** — アウティング計画の説明文を AI が生成
3. **検索・フィルタリング** — ユーザーのクエリから最適なプランを検索・推薦

**実装箇所**:
- Backend (`/server`) で Claude API と統合
- Frontend から API を呼び出し、AI の提案を表示

### AI の効果

- ⚡ **速度**: 手動計画に比べて 10 倍以上高速
- 📊 **個別化**: 参加者ごとに異なる提案
- 🎯 **正確性**: 過去データをもとに的確な提案
...
| 目標 | 達成度 | 備考 |
|-----|--------|------|
| **AI による提案機能** | ✅ | Claude API 統合済み |
| **ユーザー認証・ダッシュボード** | ✅ | Supabase 認証完了 |
