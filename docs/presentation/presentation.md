...
- **Claude API によるお出かけプラン生成API**（`POST /api/plan/generate`。本プロジェクトの核。UI画面は次フェーズで実装）
...
│  (ビジネスロジック / Claude API 統合済み: /api/plan)   │
└────────────────────┬────────────────────────────────┘
                     │ SQL
┌────────────────────▼────────────────────────────────┐
...
api/          # Vercel Functions（reservations/crowd/analytics/errors/cron/mail/plan）
scripts/      # ローカル開発用 cron
tests/        # unit / integration / e2e（jest + playwright、31本）
...
### 1. プロダクト内: Claude API によるプラン生成（T12で実装）

**実装**（`POST /api/plan/generate`、要件定義 v2 の設計をそのまま反映）:
- `backend/domains/plan/`: `claudeClient.ts`（Claude Sonnet 5 呼び出しをこの1箇所に集約）・`promptBuilder.ts`（店舗マスタ×混雑度×レビュー評価からプロンプト構築）・`scoring.ts`（距離感35%・評価25%・混雑度25%・オファー15%の重み付け）・`schema.ts`（zodによる入出力検証）
- 複数の欲求（「ランチも映画も、子連れで」）の解析・店舗DB照合・スコアリング・複数プラン案生成を **API呼び出し1回に統合**（要件定義確定事項）。Claudeの応答はzodスキーマで検証し、不正なJSONは502エラーとして扱う
- テスト: スコアリング・プロンプト構築のunitテスト、Claude APIをモックしたハンドラのintegrationテスト（6件）で正常系・異常系（JSON解釈失敗・スキーマ不一致・API呼び出し失敗）を検証
- 次フェーズ: このAPIを呼び出すチャット入力・プランカード表示UIの実装（別タスク）
...
### 15. いいね・レビュー機能の結線（T13）

❤️ **実装**:
- `LikeButton`・`StoreReviewSection` は実装済みだがどこからもimportされておらず、UIから到達不能だった問題を解消。新設した `/stores/:storeId`（`StoreDetailPage.tsx`）に両コンポーネントを組み込み
- `StoresPage.tsx` の店舗一覧each行にも `LikeButton` を追加（一覧表示時点でいいね数が見える）。店舗名クリックで詳細ページへ遷移
- `LikesListPage.tsx` から各店舗の詳細ページへの導線を追加（いいね一覧→店舗詳細へ遷移可能に）
- e2e `scenario-2-user-engagement.spec.ts` の「Scenario 2: User Engagement」ブロックを実UIに合わせて更新（`.store-item` 等の存在しないクラスから実際の `data-testid`・ボタンテキスト・`aria-label` に置換）
- 完了条件を確認: grepで `LikeButton`・`StoreReviewSection` のimport元が2ファイル以上（`StoresPage.tsx`・`StoreDetailPage.tsx`）

💡 **効果**: 「コンポーネントは存在するが機能として使えない」状態を解消。いいね・レビューがダッシュボードから実際に操作可能に

### 16. Claude API統合の受け皿実装（プラン生成ドメイン新設, T12）

🤖 **実装**:
- README・CLAUDE.md・要件定義v2のすべてがClaude API統合を前提としていたが、`@anthropic-ai`依存・API呼び出し・`ANTHROPIC_API_KEY`が実コードに一切存在しない状態を解消
- `backend/domains/plan/` を新設: `claudeClient.ts`（SDK初期化とAPI呼び出しをこの1箇所に集約。モデルは要件定義確定の`claude-sonnet-5`）・`promptBuilder.ts`（店舗マスタ・既存の混雑度整形ロジック・レビュー評価からプロンプトを構築）・`scoring.ts`（要件定義5章のスコアリング重み付け）・`schema.ts`（zodによる入出力スキーマ）
- `POST /api/plan/generate`（Vercel Function）を新設。意図解析・店舗照合・スコアリング・プラン生成をClaude API呼び出し1回に統合（要件定義の確定事項をそのまま反映）
- Claudeの応答はJSON.parse→zodスキーマ検証を経て返却。JSON解釈失敗・スキーマ不一致はいずれも502として明示的にエラーハンドリング
- `.env.example`に`ANTHROPIC_API_KEY`を追記（実キーはVercel環境変数でインフラ担当が管理。コード・.envにはコミットしない）
- オファー機能（要件定義S004）はDBスキーマが未実装のため、スコアリングでは常に「オファー無し」として扱う既知の制約として明記

💡 **効果**: プロジェクトの中核機能であるAI統合の「受け皿」が完成。ANTHROPIC_API_KEYを設定すれば即座に実データでの動作確認が可能な状態に。次フェーズはこのAPIを呼び出すチャット入力・プランカード表示UIの実装
...
- ❤️ **いいね・レビュー**（`/stores/:storeId`） — 店舗一覧・詳細ページからいいね、詳細ページでレビュー投稿・評価確認（`/likes` のいいね一覧からも詳細へ遷移可能）
...
#### 4. AI プラン提案 🚧（API実装完了・UI画面は次フェーズでデモに追加予定）
```
入力: 「ランチして映画も見たい。子連れで13時から」（POST /api/plan/generate）
出力: のんびり亭(空いてる時間帯) → つきみ座 の順路プラン＋理由（JSON、複数案）
```
...
| 目標 | 達成度 | 備考 |
|-----|--------|------|
| **AI による提案機能** | 🚧 | プラン生成API（`/api/plan/generate`）実装完了・UI画面は次フェーズ |
