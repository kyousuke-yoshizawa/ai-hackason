...
**実装手順**
1. `grep -rn "api-stubs" src/ tests/` で参照0件を再確認 → `src/lib/api-stubs.ts` を削除
...
4. `npm run build && npm test` で green を確認
5. **Marp 資料更新**
...
**問題**
1. `api/_lib/supabaseAdmin.ts` と `server/db.ts` が**ほぼ同一コードの重複**（同じ env、同じ createClient）。`server/db.ts:10` には「api/ 配下からのみ使用すること」というコメントがあるのに server/routes が import しているという自己矛盾まである
...
1. `backend/` ディレクトリを新規作成し、以下の構造で `api/_lib/` から移動:
   ...
   ```
2. `server/db.ts` を削除し、`server/routes/*.ts` の import を `backend/db.js`（相対パス `../../backend/db.js`）へ変更
...
5. **`src/hooks/useNavigate.ts` を削除**し、`LoginPage.tsx:21`・`Dashboard.tsx:19` の import を react-router-dom へ変更
...
2. ファイル名を振り直す: `002_create_stores_table.sql` → 依存順に `002a_`〜`002d_` **ではなく** `010_`〜`013_` 等の未使用番号へリネーム（既存の 003〜009 と衝突しない番号帯を使う。**ファイル内容は変更しない**）
...
`tests/e2e/scenario-2` は `/test/email-notifications` という**実在しないルート**や未結線の UI（いいね数表示）を前提にしている。e2e が「動く仕様書」ではなく「願望」になっている。T10/T13 完了時に全 e2e を実 UI と突き合わせて修正するタスクを独立で立てること。
...
要件定義 v2 は「デモの再現性を完全にコントロールできる」ことを架空エリア採用の根拠にしている。これを担保するため、**デモ用シードデータ投入スクリプト**（`scripts/seedDemoData.ts`: ことこと町の8店舗・テストユーザ・混雑パターンを一括投入）を用意するタスクを発表前週に実施すること。

### 検証終了の判断
第3パスで新規に発見される問題の重要度が Low 中心になり、既出タスクの派生who・運用系に収束したため、監査の収穫逓減点に達したと判断し、ここで打ち切る。**合計: 主要タスク22件 + 追加検証項目11件**。
